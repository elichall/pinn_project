...
int main() {
  ...
  if (fd_path == -1) {
    std::cerr << "Failed to open shared memory. Is the control loop running?"
              << std::endl;
    return 1;
  }
  IPC::PathIPC *pathIPC =
      (IPC::PathIPC *)mmap(NULL, sizeof(IPC::PathIPC), PROT_READ | PROT_WRITE,
                           MAP_SHARED, fd_path, 0);
  ...
  IPC::TelemetryIPC *telemetryIPC = (IPC::TelemetryIPC *)mmap(
      NULL, sizeof(IPC::TelemetryIPC), PROT_READ | PROT_WRITE, MAP_SHARED,
      fd_telemetry, 0);

  std::cout << "Successfully connected to POSIX shared memory." << std::endl;

  // --- Engine Initialization ---
  // CUBE instance shape, 5 objects, 800x800 window, wireframe on, sphere LOD
  // {1,1}, lines on, 5000 max line points, static color
  Engine::GraphicsEngine engine(Engine::CUBE, INSTANCE_OBJECTS_NUMBER_EST,
                                MY_WINDOW_AREA, true, {1, 1}, true,
                                MAX_PATH_POINTS, false);
  if (!engine.init()) {
  ...
  double lastTrailSampleTime = glfwGetTime();

  // --- Render Loop ---
  while (!engine.shouldClose()) {

    // --- 1. Path Data (Slow Channel) ---
    uint32_t currentPathVersion =
        telemetryIPC->pathVersion.load(std::memory_order_acquire);
    if (currentPathVersion > localPathVersion) {

      pthread_mutex_lock(&pathIPC->mutex);

      int numPoints = pathIPC->num_points;
      if (numPoints > MAX_PATH_POINTS)
        numPoints = MAX_PATH_POINTS;

      traceLines[0].points.resize(numPoints);
      for (int i = 0; i < numPoints; ++i) {
        traceLines[0].points[i] =
            glm::vec3(pathIPC->pathX[i], pathIPC->pathY[i], pathIPC->pathZ[i]);
      }
    ...
    // Sample strictly at 60Hz to prevent maxing out the buffer instantly
    double currentTime = glfwGetTime();
    if (currentTime - lastTrailSampleTime >= (1.0 / 60.0)) {
        // Only add if we haven't hit the max points yet
        if (traceLines[1].drawCount < MAX_PATH_POINTS) {
            glm::vec3 endEffectorPos = glm::vec3(robot.spatialMats[4][3]) + glm::vec3(0.0f, 0.0f, 0.1f); // Offset Z by 0.1m for visibility
